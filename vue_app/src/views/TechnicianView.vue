<template>
    <div class="home">
      ...
      <div class="columns is-mobile grid-xl">
        <!-- v-if="object.date === " -->
        <div class="column is-flex is-flex-direction-column is-align-items-center" v-if="formatDate(object.date) === formatDate(today)">  
        ...
        <div class="column is-flex is-flex-direction-column is-align-items-center">
         <div class="is-size-4 has-text-weight-bold">{{ formatDate(today) }}</div> 
  ...
  export default {
    ...
    formattedDate() {
      return this.today.toLocaleDateString(); // Format the date as 'MM/DD/YYYY' or based on locale
    }
    },
    methods: {
      onFileChange(event) {
